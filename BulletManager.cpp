...
void BulletManagerC::init()
{
	...
	/*mSpriteTextureMap = SOIL_load_OGL_texture("Sprites/bullet.png", SOIL_LOAD_AUTO, SOIL_CREATE_NEW_ID,
...
void BulletManagerC::updateSprites(DWORD milliseconds)
{
...
void BulletManagerC::randomizeSprites()
{
	for (int y = 0; y < mHeight; ++y)
	{
		for (int x = 0; x < mWidth; ++x)
...
void BulletManagerC::setMood(int x, int y, int mood)
{
	assert(x < mWidth);
	assert(y < mHeight);
	assert(mood < 4);
...
void BulletManagerC::setID(int x, int y, int id)
{
	assert(x < mWidth);
	assert(y < mHeight);
	assert(id < 8);
...
void BulletManagerC::setPosition(int x, int y, float xPosition, float yPosition)
{
	assert(x < mWidth);
	assert(y < mHeight);
...
float BulletManagerC::getXPosition(int x, int y)
{
	assert(x < mWidth);
	assert(y < mHeight);
...
float BulletManagerC::getYPosition(int x, int y)
{
	assert(x < mWidth);
	assert(y < mHeight);
...
int	BulletManagerC::getMood(int x, int y)
{
	assert(x < mWidth);
	assert(y < mHeight);
...
int	BulletManagerC::getSpriteID(int x, int y)
{
	assert(x < mWidth);
	assert(y < mHeight);
